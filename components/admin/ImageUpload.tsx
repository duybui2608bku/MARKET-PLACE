...
export default function ImageUpload({
  ...
  async function handleFileUpload(file: File) {
    ...
    if (!allowedTypes.includes(file.type)) {
      setError(t("admin.upload.error") + ": Invalid file type");
      return;
    }
    ...
    if (file.size > maxSize) {
      setError(t("admin.upload.error") + ": File too large (max 5MB)");
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.error || t("admin.upload.error"));
      }
    } catch (err) {
