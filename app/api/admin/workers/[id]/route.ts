...
import { getWorkerById, updateWorkerProfile, updateUserAdminNotes } from '@/lib/admin-workers'
...
export async function GET(
  ...
  try {
    ...
    return Response.json({ worker: result.data })
  } catch (err: any) {
    return Response.json(
 ...
 * Update worker profile or user info
 */
export async function PUT(
  ...
  try {
    ...
    // Update worker profile if there are other updates
    if (Object.keys(profileUpdates).length > 0) {
      const profileResult = await updateWorkerProfile(id, profileUpdates)
